...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!rmt_csrf_token_is_valid('catalogue', (string) ($_POST['csrf_token'] ?? '')) || $subserviceId <= 0 || $serviceId <= 0) {
        header("location:/catalogue-mgmt.php?lang={$lang}&id={$serviceId}&status=failed");
        exit();
    }
