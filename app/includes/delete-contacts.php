...
if ($_SERVER['REQUEST_METHOD']=='POST'){
	...
	$sql = "UPDATE `tblcontacts` SET `status` = '0' WHERE id='$contactid'";
	//echo $sql;
	mysqli_query($link,$sql);
	
	// Now redirect
	header("location:/contacts.php?lang=$lang&status=success"); 
	exit();
}
...
if(mysqli_num_rows($result2)>0){
	while($row2 = mysqli_fetch_array($result2)){
		$teamname = ($lang == 'fr') ? $row2['teamnamefr'] : $row2['teamnameen'];
		$title = ($lang == 'fr') ? "Supprimer le contact $teamname" : "Delete $teamname contact";
		$question = ($lang == 'fr') ? "Voulez-vous vraiment supprimer ce contact?" : "Are you sure you wish to delete this contact?";
...
<section id="filter-id" class="modal-dialog modal-content overlay-def">
	...
	<div class="modal-body">
		<form method="post" action="/includes/delete-contacts.php?lang=<?php echo $lang ?>&id=<?php echo $row2['id'] ?>">
