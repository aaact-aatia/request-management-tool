...
$translations = [
	'en' => [
		...
		'department_agency_readonly_hint' => 'Department or agency recorded during intake. This field is read-only.',
		...
		'communications_language_guidance' => 'To support WCAG language requirements, enter this communication log in English because this page is in English.',
	...
	'fr' => [
		...
		'department_agency_readonly_hint' => 'Ministère ou organisme enregistré lors de la demande initiale. Ce champ est en lecture seule.',
		...
		'communications_heading' => 'Communications',
		'communications_language_guidance' => 'Pour respecter les exigences linguistiques des WCAG, rédigez cette entrée du journal des communications en français, puisque cette page est en français.',
...
<main role="main" property="mainContentOfPage" class="container">
	...
	<?php elseif ($status == 'resolvedemailsent'): ?>
		<section id="update-status-message" class="alert alert-success" role="status" aria-live="polite" tabindex="-1">
			<h2><?php echo $t['success_heading']; ?></h2>
			<ul>
				<li><?php echo $t['resolved_email_send_success']; ?></li>
			</ul>
		</section>
	<?php elseif ($status == 'failed'): ?>
		<section id="update-status-message" class="alert alert-danger" role="alert" aria-live="assertive" tabindex="-1">
			...
			<ul>
				<li><?php echo $t['failed_message']; ?></li>
			</ul>
		</section>
	<?php elseif ($status == 'resolvedemailmissing'): ?>
		<section id="update-status-message" class="alert alert-danger" role="alert" aria-live="assertive" tabindex="-1">
			<h2><?php echo $t['failed_heading']; ?></h2>
			<ul>
				<li><?php echo $t['resolved_email_missing_client']; ?></li>
			</ul>
		</section>
	<?php elseif ($status == 'resolvedemailfailed'): ?>
		<section id="update-status-message" class="alert alert-danger" role="alert" aria-live="assertive" tabindex="-1">
			<h2><?php echo $t['failed_heading']; ?></h2>
			<ul>
				<li><?php echo $t['resolved_email_send_failed']; ?></li>
			</ul>
		</section>
	<?php endif; ?>
	...
	if (mysqli_num_rows($result) > 0) {
		...
		$intakeResult = mysqli_query($link, "SELECT id, notes FROM tblcommlog WHERE triageid = '$requestuid' AND status = '1' ORDER BY id ASC");
		...
		$departmentInputValue = rmt_department_directory_input_value($departmentOptions, $departmentAgency);
	?>
		...
		<form method="POST" enctype="multipart/form-data" action="editrequest.php?lang=<?php echo $lang; ?>&id=<?php echo $row['id']; ?>">
			...
			<div class="row">
				<div class="col-md-6">
					...
					echo renderSelect('statusid', $t['status'], $statusOptions, $row['statusid'], true, $t['select_status'], !$canEditStatusAndWorker, true);
					?>
				</div>
				<div class="col-md-6">
			...
			<fieldset>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group divsubservice">
									...
									<select class="form-control full-width" id="subserviceid" name="subserviceid" required <?php echo $readonly ? 'disabled="disabled"' : ''; ?>>
				...
				<?php if ($row['subserviceid'] == 95): ?>
					...
					<div class="row">
						<div class="col-md-6">
							<?php echo renderTextInput('sprintschedule', $t['sprint_schedule'], $row['sprintschedule'], false, $readonly, 'text', '', true); ?>
						</div>
						<div class="col-md-6">
			...
			<fieldset>
				...
				<div class="row">
					<div class="col-md-6">
						<?php echo renderTextInput('clientlname', $t['client_lastname'], $row['clientlname'], false, $readonly, 'text', '', true); ?>
					</div>
					<div class="col-md-6">
						<?php echo renderTextInput('clientfname', $t['client_firstname'], $row['clientfname'], false, $readonly, 'text', '', true); ?>
					</div>
				</div>

				<!-- Row: Client Email | Department/Agency from intake -->
				<div class="row">
					<div class="col-md-6">
						<?php echo renderTextInput('clientemail', $t['client_email'], $row['clientemail'], false, $readonly, 'email', '', true); ?>
					</div>
					<div class="col-md-6">
						<div class="form-group">
							<label for="departmentagency"><span class="field-name"><?php echo htmlspecialchars($t['department_agency'], ENT_QUOTES, 'UTF-8'); ?></span></label>
							<input
								type="text"
					...
					<div class="row">
						<div class="col-md-6">
							<?php if (in_array($catalogueid, [8, 9]) && hasValue($row['audienceid'] ?? null)): ?>
								...
								echo renderSelect('audience', $t['intended_audience'], $audienceOptions, $row['audienceid'], true, $t['select_audience'], $readonly, true);
								?>
							<?php endif; ?>
						</div>
					</div>
			...
			<div class="form-group form-buttons">
				<button type="submit" name="form_action" value="update_request" class="btn btn-primary"><?php echo $t['update_request']; ?></button>
				<a class="btn btn-default" href="viewrequest.php?lang=<?php echo $lang; ?>&erid=<?php echo base64_encode((string)$row['id']); ?>&reqid=<?php echo urlencode('a11y-' . $row['requestid']); ?>"><?php echo htmlspecialchars($t['cancel'], ENT_QUOTES, 'UTF-8'); ?></a>
			</div>
					...
					?>
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" autocomplete="url" class="form-control full-width" id="attach<?php echo $i; ?>" name="attach<?php echo $i; ?>" value="<?php echo htmlspecialchars($attachValue ?? '', ENT_QUOTES, 'UTF-8'); ?>" <?php echo $readonly ? 'readonly="readonly"' : ''; ?>>
							</div>
						</div>
					<?php endfor; ?>
				</div>
